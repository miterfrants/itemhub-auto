...
(async () => {
    const streamOfSendVerifyEmail = await sendVerifyEmail('miterfrants+robot@gmail.com');
    const respOfSendVerifyEmail = await streamOfSendVerifyEmail.json();
    const errorMessages = [];
    if (respOfSendVerifyEmail.status !== 'OK') {
        errorMessages.push(`send verify email error: ${JSON.stringify(respOfSendVerifyEmail)}`);
    }

    const streamOfVerifyEmail = await verifyEmail('miterfrants+robot@gmail.com', '000000');
    const respOfVerifyEmail = await streamOfVerifyEmail.json();
    if (!respOfVerifyEmail.token) {
        errorMessages.push(`verify email error: ${JSON.stringify(respOfVerifyEmail)}`);
    }

    const streamOfSendVerifyPhone = await sendVerifyPhone(respOfVerifyEmail.token, '0912345678', '000000');
    const respOfSendVerifyPhone = await streamOfSendVerifyPhone.json();
    if (respOfSendVerifyPhone.status !== 'OK') {
        errorMessages.push(`send verify phone error: ${JSON.stringify(respOfSendVerifyPhone)}`);
    }

    const streamOfVerifyPhone = await verifyPhone('0912345678', respOfVerifyEmail.token, '000000');
    const respOfVerifyPhone = await streamOfVerifyPhone.json();
    if (!respOfVerifyPhone.token) {
        errorMessages.push(`verify phone error: ${JSON.stringify(respOfVerifyPhone)}`);
    }

    const streamOfRegister = await register(respOfVerifyPhone.token, 'Peter', 'Huang', '@Testing123123');
    ...
    // create device
    const streamOfCreateDevice = await createDevice('robot testing', dashboardToken);
    const respOfCreateDevice = await streamOfCreateDevice.json();
    if (!respOfCreateDevice.id) {
        errorMessages.push(`create device failed: ${JSON.stringify(respOfCreateDevice)}`);
    }
    ...
    // bundle firmware
    const streamOfBundle = await bundleFirmware(respOfCreateDevice.id, dashboardToken);
    if (streamOfBundle.statusText !== 'OK') {
        errorMessages.push(`bundle fail: ${JSON.stringify(streamOfBundle)}`);
    }

    // delete device
    const streamOfDeleteDevice = await deleteDevice(respOfCreateDevice.id, dashboardToken);
...
async function notifySMS (message) {
    console.log(message);
    const streamOfSendSMS = await fetch(`https://api.e8d.tw/API21/HTTP/sendSMS.ashx`, {
	method: 'POST',
